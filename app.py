...
if page == "Record Trade":
    with st.container(border=True):
        ...
        with c3:
            # Optional exit inputs (for closed positions)
            closed_flag = st.checkbox("Closed trade?")
            exit_date = None
            if closed_flag:
            ...
            currency = st.selectbox("Trade Currency", ["INR", "USD", "AUD"],
                                    index=["INR","USD","AUD"].index(default_ccy))
    ...
    with st.container(border=True):
        ...
        st.caption("Provide a planned trade to get suggested SL/targets & sentiment. "
                   "Requires OpenAI key in secrets or environment; else uses rule-based fallback.")
        ...
        with cc3:
            risk_perc = st.slider("Baseline Risk %", 0.2, 3.0, 1.0, 0.1,
                                  help="Used by the heuristic when LLM is unavailable.")
...
# -------------------- Page: Monthly Report --------------------
    ...
    if df.empty:
        ...
        if not month_opts:
            st.info("No dated entries yet. Add at least one trade with an Entry Date.")
        else:
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                sel_month = st.selectbox("Month (by entry)", month_opts, index=len(month_opts) - 1)
            with col2:
                rep_ccy = st.selectbox("Reporting Currency (base)", ["AUD", "USD", "INR"],
            ...
            with cA:
                ...
                if not cur_df.empty:
                    ...
                    st.write("No P&L yet for this month.")
            ...
            st.dataframe(mdf.sort_values("entry_date", ascending=False),
...
# -------------------- Page: Settings --------------------
    ...
    bc = st.selectbox("Base currency for reporting", ["AUD", "USD", "INR"],
                      index=["AUD","USD","INR"].index(settings["base_currency"]))
    # FX to base mapping
    st.caption("**FX to Base**: how many units of BASE currency for 1 unit of the listed currency "
               "(e.g., base AUD: USD→AUD=1.55; INR→AUD=0.0185).")
